// js/state.js — Глобальний стан та допоміжні функції
...
const REQUESTS = new Map(); // uid → об'єкт користувача (вхідні запити)
...
/**
 * Відносний час (щойно, 5 хв тому, вчора тощо)
 */
function fmtTime(ts) {
  ...
  if (min < 60) return min + ' ' + t('time.min', { n: min });
  if (hour < 24) return hour + ' ' + t('time.hour', { n: hour });
  if (day === 1) return t('time.yesterday');
  return day + ' ' + t('time.day', { n: day });
}

/**
 * Форматування чисел (К, М)
 */
function fmtN(n) {
  if (!n || n === 0) return '';
...
/**
 * Екранування тексту для захисту від XSS
 */
function esc(s) {
...
/**
 * Перетворює хештеги та згадки у клікабельні посилання
 */
function tags(txt) {
  return txt
...
/**
 * Зберігає дані користувача (Async для майбутнього Supabase)
 */
