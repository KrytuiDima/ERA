// js/feed.js — Рендеринг стрічки та взаємодія з постами

// ── Вкладки стрічки ─────────────────────────────────────────────
/**
 * Перемикає тип стрічки (Для тебе / Підписаний)
 */
...
/**
 * Повертає список постів для поточної стрічки з урахуванням алгоритмів
 */
function getFeedPosts() {
  ...
  // Для тебе: враховуємо настрій та друзів
  if (APP.mood) {
...
/**
 * Рендерить пости у контейнер стрічки
 */
function renderFeed() {
  ...
  // Ініціалізуємо каруселі після рендерингу
  setTimeout(() => {
...
// ── Картка поста ─────────────────────────────────────────────
/**
 * Створює HTML структуру для одного поста
 */
function renderPostCard(post, delay = 0) {
  ...
  // Маркер друга з кольором власного вайбу юзера
  ...
  <div class="post-head">
    ...
    <div class="vibe-dot" style="background:${frnd ? myCols[0] : cols[0]}${frnd ? ';box-shadow:0 0 8px ' + myCols[0] : ''}; ${frnd ? 'outline: 1.5px solid ' + myCols[0] : ''}"></div>
  </div>
...
function refreshLikeBtn(pid) {
  ...
  if (lbLb) {
    lbLb.className = 'like-btn' + (p.liked ? ' liked' : '');
    lbLb.querySelector('svg')?.setAttribute('fill', p.liked ? 'currentColor' : 'none');
    const cnt = lbLb.querySelector('.like-cnt');
    if (cnt) cnt.textContent = fmtN(p.likes) || 0;
  }
}

// ── Кліки та дабл-тапи (Taps & Double-taps) ──────────────────────
...
/**
 * Ставить лайк з анімацією серця по центру
 */
...
// ── Поширення та видалення (Post actions) ──────────────────────────
...
function openCmts(pid) {
  ...
  ${p.comments.length === 0 ? `<div id="cmt-empty" style="text-align:center;font-size:12px;color:var(--t3);padding:16px 0">${t('post.firstComment')}</div>` : ''}`;

  openModal('modal-cmt');
}

/**
 * Рендерить один коментар з підтримкою медіа
 */
function renderCmt(c) {
  ...
  <div class="cmt-bwrap">
    ...
    ${c.photo ? `<img class="cmt-photo-img" src="${c.photo}" alt="" onclick="expandPhoto('${c.photo}')" style="cursor:pointer; width:70px; height:70px; object-fit:cover; border-radius:10px; margin-top:6px; border:1px solid var(--b1)">` : ''}
  </div></div>`;
}

/**
 * Відкриває фото коментаря в лайтбоксі
 */
function expandPhoto(src) {
  ...
  lb.innerHTML = `
    ...
    <div class="lb-wrap" style="align-items:center;justify-content:center;height:100%;touch-action:auto">
      <img src="${src}" style="max-width:100%;max-height:92vh;border-radius:12px;box-shadow:0 0 40px rgba(0,0,0,0.5)">
    </div>`;
...
function onCmtFile(e) {
  ...
  r.onload = ev => {
    CMT_PHOTO = ev.target.result;
    document.getElementById('cmt-ph-img').src = CMT_PHOTO;
    document.getElementById('cmt-ph-prev').classList.remove('hidden');
  };
  r.readAsDataURL(f);
}
...
async function sendCmt() {
  ...
  const empty = document.getElementById('cmt-empty'); if (empty) empty.remove();
