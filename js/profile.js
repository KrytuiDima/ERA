// js/profile.js — Профілі користувачів

/**
 * Рендерить основну сторінку профілю
 */
async function renderProfile(uid) {
  APP.profileUid = uid;
  const u = getUser(uid);
  const own = APP.user && u.id === APP.user.id;
  ...
  // Отримуємо пости користувача (async в майбутньому)
  const myPosts = POSTS.filter(p => p.userId === uid);

  // Сортування: спочатку закріплені пости
  ...
  // Перевірка приватності: бачимо контент, якщо акаунт публічний, або ми власник, або ми підписані (approveAction 1)
...
<div class="profile-info">
  <div class="profile-ava-row">
    <div class="profile-ava" ${own ? `onclick="document.getElementById('pava-f').click()" title="${t('profile.changePhoto')}"` : ''}>
      <div class="profile-ava-glow" style="background:linear-gradient(135deg,${frnd ? myCols[0] : cols[0]},${frnd ? myCols[1] : cols[1]},${frnd ? myCols[2] : cols[2]});${frnd ? 'filter:blur(10px);opacity:.9;animation:none' : ''}"></div>
      <div class="profile-ava-ring" style="${frnd ? 'border-color:' + myCols[0] : ''}">${avatarHTML(u, 84, { friend: false })}</div>
...
${!canSee
    ? `<div class="empty-state">
         ...
         <div class="empty-txt" style="font-weight:600;font-size:15px;color:var(--t1)">Це приватний акаунт</div>
         <div class="empty-txt">Підпишись на користувача, щоб бачити його пости та медіа.</div>
       </div>`
    : sorted.length === 0
...
<div class="uc-cover">
  <div class="uc-cover-inner">${u.banner ? `<img src="${u.banner}" style="width:100%;height:100%;object-fit:cover">` : makeVibeCode(u.baseColor, u.id, 400, 80)}</div>
...
<div class="uc-body">
  ...
  ${u.bio ? `<div style="font-size:12px;color:var(--t2);line-height:1.5;margin-bottom:12px">${esc(u.bio).slice(0, 150)}</div>` : ''}
