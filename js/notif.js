...
function renderNotifRow(n, idx) {
  ...
  <!-- Зона B: Текст -> Пост -->
  <div style="flex:1; min-width:0; cursor:pointer" onclick="${post ? `expandPost('${post.id}', null, '${n.type==='comment'?n.cmtId:''}')` : ''}">
  ...
  ${post ? `<div style="width:40px; height:40px; border-radius:8px; overflow:hidden; flex-shrink:0; background:${postBg}; cursor:pointer" onclick="expandPost('${post.id}', null, '${n.type==='comment'?n.cmtId:''}')">
