...
// Групування сповіщень: якщо один юзер лайкнув кілька постів, згортаємо в один рядок
function _groupNotifs(notifs) {
  const out = [], seen = new Set();
  for (const n of notifs) {
    if (seen.has(n.id)) continue;

    // Групуємо тільки лайки від одного і того ж користувача
    if (n.type === 'like') {
      const sameUser = notifs.filter(x => x.userId === n.userId && x.type === 'like' && !seen.has(x.id));
      if (sameUser.length > 1) {
        const merged = {
          ...sameUser[0],
          _count: sameUser.length,
          _ids: sameUser.map(x => x.id),
          unread: sameUser.some(x => x.unread) // Якщо хоч одне не прочитане — вся група не прочитана
        };
...
function renderNotifRow(n, idx) {
  ...
  // Скрол до коментаря, якщо сповіщення про комент
  const expandFn = n.type === 'comment' ? `expandPost('${post.id}'); setTimeout(() => { const el = document.getElementById('cmt-list')?.lastElementChild; if(el) el.scrollIntoView({behavior:'smooth'}); }, 400)` : `expandPost('${post.id}')`;
  ...
  <!-- Зона B: Текст -> Пост -->
  <div style="flex:1; min-width:0; cursor:pointer" onclick="${post ? expandFn : ''}">
