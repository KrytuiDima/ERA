// js/notif.js — Інтелектуальний центр сповіщень (Smart Notifications)
// ERA групує однотипні дії та використовує персоналізовані маркери новизну.
...
async function renderNotif() {
  // Групуємо повторювані дії (напр. кілька лайків від одного юзера)
...
${grouped.length === 0
  ...
  // МАРКЕРИ НОВИЗНИ: Автоматично позначаємо всі як прочитані при перегляді.
  // Маркери зникнуть при наступному рендері.
  NOTIFS.forEach(n => n.unread = false);
  renderNotifBadge();
}

/**
 * АНТИ-СПАМ ГРУПУВАННЯ: Якщо один юзер лайкнув кілька постів підряд,
 * це згортається в один рядок: "@user лайкнув 5 ваших постів".
 */
function _groupNotifs(notifs) {
  ...
  for (const n of notifs) {
    ...
    // Групуємо лайки від одного і того ж користувача
    if (n.type === 'like') {
 ...
 * ЧІТКЕ ЗОНУВАННЯ КЛІКІВ:
 * - Клік на Аватар/Нікнейм -> профіль користувача.
 * - Клік на текст або прев'ю -> відкриття поста у Лайтбоксі.
 * - Клік на кнопки -> миттєві дії (Action 1/2).
 */
function renderNotifRow(n, idx) {
  ...
  // ПЕРСОНАЛІЗОВАНИЙ МАРКЕР: колір ВЛАСНОГО вайбу юзера.
  ...
  // Формування тексту на основі типу
  let text = '';
  ...
  switch (n.type) {
    ...
    case 'follow':
      ...
      break;
    case 'request':
      text = n._approved ? t('notif.followed', { user: u.username }) : t('notif.wantsToFollow', { user: u.username });
      break;
    case 'approved':
  ...
  // КНОПКИ ДІЙ ДЛЯ ПРИВАТНИХ ПРОФІЛІВ:
  // Дія 1: "Дозволити перегляд" (для запитів)
  ...
  <div style="flex:1; min-width:0; cursor:pointer" onclick="${post ? `expandPost('${post.id}')` : `renderFullProfile('${u.id}')`}">
    ...
    <div style="font-size:10px; color:var(--t2); margin-top:3px">${fmtTime(n.ts)}</div>
    ${actionBtns}
  </div>
