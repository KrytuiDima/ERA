...
function showLangPicker() {
  ...
  el.innerHTML = `
    <div class="sheet" style="max-width:320px; border-radius:20px; border-bottom:1px solid var(--b2)">
      <div class="sheet-hdr"><span class="sheet-title">🌐 Language / Мова</span></div>
      <div class="sheet-body" style="padding:10px">
      ${LANGUAGES.map(l => {
        const active = l.code === currentCode;
        return `<button onclick="switchLang('${l.code}');eraBack()"
  ...
  el.addEventListener('click', e => { if (e.target === el) eraBack(); });
