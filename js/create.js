...
async function onPostFile(e) {
  ...
  /**
   * Обробка файлів для поста з послідовним кропом кожного фото
   */
  for (let i = 0; i < files.length; i++) {
    const src = await readFile(files[i]);
    // Використовуємо Promise, щоб дочекатися завершення кропу кожного фото
    await new Promise(resolve => {
      showCropTool(src, (cropped) => {
        POST_IMGS.push(cropped);
        if (POST_IMGS.length > 10) POST_IMGS = POST_IMGS.slice(0, 10);
        renderUploadGrid();
        resolve();
      }, { ratio: 4/5 });
    });
    // Якщо користувач додав забагато фото — зупиняємось
